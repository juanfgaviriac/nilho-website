...
5. An APPROVED matching order is durably claimed. Two independent messages are frozen and sent through Resend: the buyer receipt and a “Nueva compra Foco” alert to `team@nilho.co`. Each has its own durable record (`receipts/` or `alerts/`), message ID, lease, conditional writes and deterministic provider key. Either may retry without resending the successful message. Unresolved sends older than 23 hours require manual reconciliation, never automatic resend.
...
- Sales alerts go directly to `team@nilho.co`, configured server-side as `FOCO_MERCHANT_EMAIL` in `server/foco/email.mjs`; no new environment variable is needed. The email contains quantity, total, order reference, transaction ID and the Wompi dashboard link. Customer contact/shipping/payment data is not copied. Sandbox alerts are labelled `[PRUEBA]` and “NO ES UNA COMPRA”; the existing sandbox-recipient restriction applies before either message is prepared.

...
export async function snapshotPolicies() {
    for (const [kind, source, version] of [['terms','foco/compra/index.html',c.termsVersion], ['privacy','foco/compra/privacidad/index.html',c.privacyVersion]]) {
        ...
        html = html.replace(/\s*<script[^>]*src="\/foco\/commerce.js[^>]*><\/script>/g, '')
            .replace(/\s*<p[^>]*data-preview[^>]*>[\s\S]*?<\/p>/g, '')
            .replace(/(<[^>]+data-field="([^"]+)"[^>]*>)[^<]*(<\/[^>]+>)/g, (_,open,key,close) => `${open}${escapeHTML(fields[key])}${close}`)
            ...
            .replace('<div class="commerce-offers" data-offer-list></div>', `<div class="commerce-offers">${[1,2,3].map(q=>{const o=getOffer(q);return `<div><strong>${offerName(q)}</strong><span>${formatCOP(o.total)} COP</span><small>${o.shipping ? `${formatCOP(o.shipping)} de envío incluido` : 'Envío gratis'}${o.discount ? ` · Ahorras ${formatCOP(o.discount)}` : ''}</small></div>`}).join('')}</div>`);

...
**Still required before website deployment or enabling its payment button:**

- Publish and verify the configured return page, `https://nilho.co/foco/pago/`, with checkout; it currently exists only in this branch.
- Merchant choice: persona natural, as requested by the owner. The live Wompi merchant is Foco; final onboarding/payout approval is not yet verified. Confirm the seller identity shown in checkout/policies matches Wompi and invoicing before launch. Never commit or request secrets in chat.
...
- Complete final review and obtain explicit approval before setting `productionEnabled=true` or deploying. The production links are configured; the website payment button is still disabled. Do not push to auto-deploying `main` yet.
